...
from bsc_sommer_project_module.cfm import cfm_t_xt_ut, custom_wrapper, CondVF
...
# DataLoaders for MC
base_train_loader = DataLoader(base_train_tensor, batch_size=batch_size, shuffle=True, drop_last=True)
...
conditions_train_loader = DataLoader(conditions_train_tensor, batch_size=batch_size, shuffle=True, drop_last=True)
...
for epoch in range(n_epochs):
    ...
    for step, (base_batch, conditions_batch, target_batch) in enumerate(zip(base_train_loader, conditions_train_loader, target_train_loader)):
        ...
        t, xt, ut = cfm_t_xt_ut(base_batch, target_batch)

        #concatenate the sample location xt, the conditions_batch and t for the model input
        model_input = torch.cat([xt, conditions_batch, t], dim=1)
    ...
    with torch.no_grad():
        for step, (base_batch, conditions_batch, target_batch) in enumerate(zip(base_val_loader, conditions_val_loader, target_val_loader)):
            ...
            #node = NeuralODE(custom_wrapper(model), solver="dopri5", sensitivity="adjoint", atol=1e-4, rtol=1e-4)
            #morphed_batch = node(base_batch)
            ...
            def ode_func(t, x):
                ...
                inp = torch.cat([x_flat, conditions_batch, t_tensor], dim=1)
            ...
            t_span = torch.tensor([0.0, 1.0], device=base_batch.device)
            traj = odeint(
                ode_func,
            ...
            morphed_batch = traj[-1].squeeze(1)
