...
def load_config():
    ...
    parser.add_argument('--scheduler_patience', type=int, default=12, help='Patience for the learning rate scheduler')
    ...
    parser.add_argument('--save_progress', type=bool, default=False, help='decide whether to save the progress of the training')
    parser.add_argument('--hidden_dimensions', type=int, default=4096, help='Number of hidden dimensions in the model')
...
#standardize base usiing mc mean and std
...
#%%
#save the loss to the plotting directory
year = 2024
...
lossplot_train(array_loss_train, array_loss_val, epoch+1, best_val_epoch, filename=f"loss_plot_best_epoch_{best_val_epoch+1}.png", out_dir=outdir)
print("Saved loss plot to:", outdir)

# Prepare payload and call plot helper
# Move test tensors to CPU and detach
base_test_cpu       = base_test_tensor.detach().cpu()
conditions_test_cpu = conditions_test_tensor.detach().cpu()
target_test_cpu     = target_test_tensor.detach().cpu()
weights_test_cpu    = weights_test_tensor.detach().cpu()

data_test_df = data_df.iloc[test_indices_cms]
mc_test_df = mc_df.iloc[test_indices_mc]

# Build payload with tensors and corresponding column names
payload = {
    "base_tensor":       base_test_cpu,
    "conditions_tensor": conditions_test_cpu,
    "target_tensor":     target_test_cpu,
    "weights_tensor":    weights_test_cpu,
...
helper_script = Path(__file__).parent.parent / "plotting" / "plot_helper.py"
cmd = [
