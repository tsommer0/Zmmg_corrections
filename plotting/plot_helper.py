...
# …all your imports and load_config()/model‐init code up here…

# Right before unpickling, add a debug print to stderr:
...
try:
    ...
    base_tensor        = payload["base_tensor"]
    conditions_tensor  = payload["conditions_tensor"]
    weights_tensor     = payload["weights_tensor"]
...
# If you see “child: got keys …” in stderr, you know the unpickle succeeded.
# From here you can convert to DataFrames and proceed with plotting.
...
'''
#IRRELEVANT. REPLACED BY IMPORTING THE PICKLED DATAFRAMES X_text_df USED BELOW
# Build DataFrames from the tensors unpacked earlier
data_df = pd.DataFrame(target_tensor.detach().cpu().numpy(), columns=variables_cms_names)
mc_df = pd.DataFrame(base_tensor.detach().cpu().numpy(), columns=variables_mc_names)
conditions_df = pd.DataFrame(conditions_tensor.detach().cpu().numpy(), columns=conditions_names)
mc_df = mc_df.join(conditions_df)
mc_df["weights"] = weights_tensor.detach().cpu().numpy()
#IRRELEVANT. REPLACED BY IMPORTING THE PICKLED DATAFRAMES X_text_df USED BELOW
'''
#%%
...
#%%
#overwrite the use_x_df dataframes with the ones from the mother script
#use_data_df = data_test_df
#use_mc_df = mc_test_df
